...
`summary(splineAll)` would tell us that the `r nrow(splineAll)` help pages came from `r nPacSplineAll` different packages and display the first `minPackages = 12` such packages. (If other packages had the same number ofm matches as the twelfth package, they would also appear in this summary. `minPackages is an argument of the `summary.findFn` function and can be changed if the user wishes.) 
...
The lead author of this article recently gave an invited presentation on "Fitting Nonlinear Differential Equations to Data in R"^[[Workshop
on Statistical Methods for Dynamic System Models, Vancouver, 2009: http://stat.sfu.ca/~dac5/workshop09/Spencer_Graves.html](http://people.stat.sfu.ca/~dac5/workshop09/Spencer_Graves.html). A key part of preparing for that presentation was a search of
